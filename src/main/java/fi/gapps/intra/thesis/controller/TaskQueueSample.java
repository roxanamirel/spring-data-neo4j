...
/**
 * Command-line sample which leases tasks from a pull task queue, processes the
 * payload of the task and then deletes the task.
 */
public class TaskQueueSample {
	 ...
	 */
	private static final String APPLICATION_NAME = "spacegapps";
	...
	public static boolean parseParams() {
		try {
			projectId = "spacegapps";
			taskQueueName = "pull-queue";
			leaseSecs = 10;
			numTasks = 1;
			return true;
			...
			System.out.println("Please specify lease seconds " + "and Number of tasks to lease, in number " + "format");
			return false;
		}
	}
...
//	}
	 ...
	 *
	 * @param taskQueue
	 *            The task queue that should be used to get the queue from.
	 * @return {@link com.google.api.services.taskqueue.model.TaskQueue}
	 * @throws IOException
	 *             if the request fails.
	 */
	private static com.google.api.services.taskqueue.model.TaskQueue getQueue(Taskqueue taskQueue) throws IOException {
	 ...
	 *
	 * @param taskQueue
	 *            The task queue that should be used to lease tasks from.
	 * @return {@link Tasks}
	 * @throws IOException
	 *             if the request fails.
	 */
	...
	/**
	 * This method actually performs the desired work on tasks. It can make use
	 * of payload of the task. By default, we are just printing the payload of
	 * the leased task after converting it from Base64 encoding.
	 *
	 * @param task
	...
	/**
	 * Method that sends a delete request for the specified task object to the
	 * taskqueue service.
	 *
	 * @param taskQueue
	 *            The task queue the specified task lies in.
	 * @param task
